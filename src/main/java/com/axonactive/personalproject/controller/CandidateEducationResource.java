...
public class CandidateEducationResource {
  ...
  @GetMapping("/{id}")
  public ResponseEntity<CandidateEducationDto> getById(@PathVariable("id") Integer id)
      throws EntityNotFoundException {

    return ResponseEntity.created(URI.create(PATH + "/" + id))
        .body(candidateEducationService.findById(id));
  }

  @PostMapping
  public ResponseEntity<CandidateEducationDto> add(@RequestBody CandidateEducationRequest inputData)
      throws EntityNotFoundException {
    CandidateEducation newCandidateEducation = candidateEducationService.add(inputData);
